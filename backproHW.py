...
w2 = torch.Tensor([1.0])     # 初始化权重
w2.requires_grad = True      # 表明w需要计算梯度

b = torch.Tensor([1.0])     # 初始化权重
b.requires_grad = True      # 表明w需要计算梯度

# define the linear model
def forward(x):
    return x * w * x + w2 * x + b            # 这里会构建计算图 将x转化成Tensor
...
for epoch in range(100):
    for x, y in zip(x_data, y_data):
        ...
        print('\tgrad:', x, y, w.grad.item(), w2.grad.item(), b.grad.item())   # 将梯度存到w之中,随后释放计算图 item()可以直接将梯度变成标量
